...
export class FacultyLogin extends React.Component {
  ...
  componentDidMount(){
    ...
    if(obj && obj.token) {
      const { token } = obj;
      fetch('/api/facultyaccount/verify', {
        method: 'POST',
        headers: {
      ...
      .then(json => {
        if(json.success){
          ...
          console.log('token is valid faculty');
        } else {
          this.setState({
            isLoading: false
          });
  ...
  onLogin() {
    ...
    fetch('/api/facultyaccount/signin', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    .then(json=>{
      if(json.success){
        setInStorage('faculty_obj', {token: json.token, empId: signInEmpId, branch: json.branch});
        this.setState({
          signInError: json.message,
          isLoading: false,
          token: json.token,
        });
        const { from } = this.props.location.state || { from: {pathname: "/faculty"}};
